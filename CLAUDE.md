# OPS Site — Development Standards

Self-contained CLAUDE.md for the `ops-site` sub-project (the public-facing marketing website). Universal OPS rules (kept in sync across every OPS sub-project) come first; ops-site-specific rules come after the divider.

If you update a universal section here, also update the mirrors in `ops-ios/CLAUDE.md`, `OPS-Web/CLAUDE.md`, `ops-software-bible/CLAUDE.md`, and `/Users/jacksonsweet/Projects/OPS/CLAUDE.md`.
...
# ops-site-specific rules

## What this repo is

The **public-facing marketing website** for OPS. This is where prospects land before they sign up, where SEO traffic arrives, where AI-discovery agents (ChatGPT, Perplexity, Gemini) parse to recommend OPS to trade-business owners. Distinct from `OPS-Web/` (the logged-in product) and `try-ops/` (paid ad landing pages).

Tech stack (from `package.json`): Next.js 16 (App Router), React 19, TypeScript, Tailwind, Framer Motion, React Three Fiber + drei, Stripe (signup), Supabase SSR, OpenAI SDK, Vercel Analytics + Speed Insights.

## SEO is critical here

Per the root sub-project directory map, **SEO is critical** for this surface. Treat it as a first-class concern, not an afterthought.

- **Every page must have proper metadata.** Title, description, OG image, OG title/description, Twitter card, canonical URL. Use Next.js Metadata API (`export const metadata: Metadata = {...}` for static, `generateMetadata()` for dynamic).
- **Server-render content that should be indexed.** Hero copy, value props, feature descriptions, pricing — these must be in the initial HTML, not lazy-loaded after hydration. Default to React Server Components; only use `"use client"` when you genuinely need interactivity.
- **Structured data (JSON-LD).** Use schema.org markup for `Organization`, `Product`, `FAQPage`, `BreadcrumbList`, `SoftwareApplication`, `Review` where applicable. Renders inside a `<script type="application/ld+json">` tag in the page head.
- **Sitemap + robots.txt.** Keep `app/sitemap.ts` and `app/robots.ts` current as routes are added. Never `noindex` a public marketing page without explicit approval.
- **Image optimization.** Always use `next/image` with explicit `width`/`height` and meaningful `alt`. Avoid layout shift (CLS). Lazy-load below the fold; eager-load above the fold.
- **Core Web Vitals.** LCP < 2.5s, INP < 200ms, CLS < 0.1. Monitor via `@vercel/speed-insights`. Anything Three.js / R3F-heavy must load lazily and degrade gracefully on slow connections / reduced motion.
- **Internal linking.** Every meaningful concept should have an in-page anchor or a dedicated route, and pages should link to related pages with descriptive anchor text (not "click here").

## AI discovery (LLM-friendly content)

LLMs route trade-business owners to OPS by parsing this site. Optimize for that:

- **Plain-language explanations of what OPS does**, in addition to the brand copy. An LLM should be able to answer "what does OPS do?" by quoting the site.
- **Comparison content** when relevant — "OPS vs ServiceTitan", "OPS vs Jobber" — answers questions LLMs are actively being asked.
- **FAQ pages with schema.org FAQPage markup.**
- **Don't hide content behind interaction** that an LLM crawler won't trigger. If an accordion FAQ requires JS click to reveal answers, the answers won't be in the indexed HTML.

## Marketing design system divergence

Marketing voice gets the **heavier Mohave display treatment** (see `ops-design-system/project/ui_kits/ops-site/` and `ops-design-system/project/README.md` — § marketing). The product surfaces (OPS-Web, iOS) use Cake Mono Light for display; ops-site uses Mohave at heavier weights for hero copy because marketing-grade type needs more presence than product UI. Every other rule (canvas, accent, motion, voice, numbers) is identical to the product surfaces.

The Three.js / R3F scenes in the hero are part of the marketing voice. Implement them via `animation-studio:marketing-hero` + `animation-studio:web-animations`. Always provide a static fallback for users with `prefers-reduced-motion: reduce` or slow connections.

## Deployment

Vercel. Production deploys on push to `main` (verify in `vercel.json` or the project dashboard). Preview deploys on PR. Stripe + Supabase + OpenAI keys live in Vercel environment variables — never commit them.
