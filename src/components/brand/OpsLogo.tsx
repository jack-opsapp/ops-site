 ...
 *
 * Renders the full lockup (mark + "OPS" wordmark) as a single inline SVG inside
 * a width-constrained container. When `collapsed={true}`, the container shrinks
 * to the mark's width; the SVG itself doesn't move, so the mark stays anchored
 * at the left edge and the wordmark appears to wipe in/out from the right.
 *
 * Inherits `fill="currentColor"` from parent CSS text color — set color via
 * `text-*` classes on the enclosing element.
 *
 * Easing + duration follow spec v2 (`cubic-bezier(0.22, 1, 0.36, 1)`, 300ms
 * panel-transition tier). No spring physics, no bounce.
 */
...
// Mark bounding box in the lockup's viewBox coords: x=297–827, y=311–1199.
// We expand the container to include ~half the gap between mark and wordmark so
// the collapsed state reads as balanced (not crammed against the mark's edge).
const MARK_END_VB_X = 870; // mark ends at 827; +43 of the 302-unit gap for balance
const MARK_COLLAPSED_ASPECT = MARK_END_VB_X / LOCKUP_VB_H; // ~0.576

export interface OpsLogoProps {
  /** Logo height in px. Controls both container height and SVG render size. */
  size?: number;
  /** When true, container shrinks to mark-only width (wordmark clipped). */
  collapsed?: boolean;
  /** Transition duration in ms. Default 300 (spec panel tier). */
  durationMs?: number;
  /** Accessible label. Defaults to "OPS". */
  title?: string;
  className?: string;
}
    ...
    return (
      ...
      >
        ...
        <svg
          ...
          style={{ display: 'block' }}
        >
          {/* Mark — two interlocking chamfered brackets (stays anchored at left,
              always fully opaque) */}
          <g>
          ...
          {/* Wordmark "OPS" (O / P / S) — opacity animates alongside the container
              width so the unveil/tuck reads as a dissolve rather than a hard crop */}
          <g
            style={{
