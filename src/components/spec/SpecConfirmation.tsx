...
export default function SpecConfirmation({ dict }: { dict: Dictionary }) {
  const searchParams = useSearchParams();
  const sessionId = searchParams.get('session_id');
  const [session, setSession] = useState<SessionData | null>(null);
  // No session_id in the URL means there's nothing to fetch — short-circuit
  // initial loading state instead of toggling it inside the effect, which
  // tripped react-hooks/set-state-in-effect on Next 16.
  const [loading, setLoading] = useState<boolean>(!!sessionId);

  useEffect(() => {
    if (!sessionId) return;
    let cancelled = false;
    fetch(`/api/spec/checkout-session?session_id=${sessionId}`)
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return;
        setSession(data);
