...
interface UseAutoRotationParams {
  ...
  /** Concrete identity signal so listener ownership follows a replacement. */
  controlsInstance?: OrbitControlsImpl | null;
  /** Legacy readiness signal for callers that have not adopted identity tracking. */
  controlsReady?: boolean;
  isMobile: boolean;
  enabled: boolean;
  /** Immediate imperative ownership gate for the scroll journey. */
  blockedRef?: React.RefObject<boolean>;
  isHoveringPhone?: boolean; // Raycast-based hover from PhoneInteraction
}

export function useAutoRotation({ controlsRef, controlsInstance, controlsReady, isMobile, enabled, blockedRef, isHoveringPhone = false }: UseAutoRotationParams) {
  ...
  useFrame((_, delta) => {
    ...
    if (!controls || !enabled || blockedRef?.current) return;
  ...
  // The concrete instance is the dependency signal. A boolean readiness flag
  // cannot distinguish a non-null → different non-null replacement, which
  // would leave drag listeners attached to the obsolete controls object.
  useEffect(() => {
    const controls = controlsInstance === undefined
      ? controlsReady
        ? controlsRef.current
        : null
      : controlsInstance;
  ...
  }, [controlsInstance, controlsReady, controlsRef, isMobile, updateTargetSpeed]);
