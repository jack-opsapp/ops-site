/**
 * SPEC Packages Screen — Three package tiers with real text.
 */
...
export function drawSpecPackages({ ctx, width, height, progress, tier }: SpecScreenDrawParams) {
  const cw = width - P * 2;
  const lx = P + CP;
  const rx = P + cw - CP;
  ...
  drawText(ctx, 'CHOOSE YOUR PACKAGE', P, TOP_INSET, FONTS.caption, SPEC_COLORS.accentText, structP);
  drawText(ctx, 'SPEC Packages', P, TOP_INSET + 38, FONTS.title, TEXT.primary, structP);
  ...
  for (const pkg of packages) {
    ...
    const borderColor = isSelected ? SPEC_COLORS.accentBorder : COLORS.border;
    const fillColor = isSelected ? SPEC_COLORS.accentGlow : COLORS.cardFill;
    ...
    if (isCompressed) {
      ...
      pkg.highlights.forEach((h, hi) => {
        ...
        drawCheck(ctx, lx + 2, hy + 2, 10, SPEC_COLORS.accentTextDim, contentP);
      ...
      if (pkg.recommended) {
        ...
        roundedRect(ctx, rx - badgeW, cardY + 100, badgeW, 30, 3,
          SPEC_COLORS.accentGlow, SPEC_COLORS.accentBorder, contentP);
        drawText(ctx, 'RECOMMENDED', rx - badgeW / 2, cardY + 115, FONTS.labelXs,
          SPEC_COLORS.accentText, contentP, 'center');
      }

    } else {
      // ── Expanded ──
      drawText(ctx, pkg.label, lx, cardY + 38, FONTS.caption, SPEC_COLORS.accentText, contentP);
      ...
      pkg.features.forEach((feat) => {
        drawCheck(ctx, lx + 2, fy + 2, 12, SPEC_COLORS.accentText, accentP);
      ...
      drawText(ctx, pkg.example.trade, lx + calloutInset, fy + 30, FONTS.labelXs, SPEC_COLORS.accentText, accentP);
      ...
      // CTA button
      roundedRect(ctx, lx, fy, cw - CP * 2, 52, 3, SPEC_COLORS.accentOverlayStrong, undefined, accentP);
