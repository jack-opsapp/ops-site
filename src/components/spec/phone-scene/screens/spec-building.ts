/**
 * SPEC Building Screen — Modules being assembled with real names.
 */
...
export function drawSpecBuilding({ ctx, width, height, progress }: SpecScreenDrawParams) {
  const cw = width - P * 2;
  const lx = P + CP;
  ...
  // Header
  drawText(ctx, 'BUILDING', P, TOP_INSET, FONTS.caption, SPEC_COLORS.accentText, structP);
  ...
  customModules.forEach((name, i) => {
    ...
    drawText(ctx, name, lx + 28, modY + modH / 2, FONTS.bodyMed, SPEC_COLORS.accentText, modP);
    modY += modH + 12;
  });
