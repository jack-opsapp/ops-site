/**
 * SPEC Analysis Screen — Workflow diagram with labeled steps.
 */
...
export function drawSpecAnalysis({ ctx, width, height, progress }: SpecScreenDrawParams) {
  const cw = width - P * 2;
  ...
  // Header
  drawText(ctx, 'ANALYZING', P, TOP_INSET, FONTS.caption, SPEC_COLORS.accentText, structP);
  ...
  steps.forEach((step, i) => {
    ...
    if (i > 0 && structP > 0) {
      ...
      ctx.strokeStyle = isCustom ? SPEC_COLORS.accentBorder : COLORS.separator;
      ctx.lineWidth = 2;
      ctx.setLineDash([5, 5]);
      ctx.beginPath();
      ...
      ctx.stroke();
      ctx.setLineDash([]);
      ctx.fillStyle = isCustom ? SPEC_COLORS.accentBorder : COLORS.separator;
      ctx.beginPath();
    ...
    if (isCustom) {
      ...
      roundedRect(ctx, boxX, stepY, boxW, boxH, LAYOUT.cardRadius,
        SPEC_COLORS.accentGlow, SPEC_COLORS.accentBorder, cP);
      ...
      if (cP > 0) {
        ...
        ctx.save();
        ctx.globalAlpha = cP;
        ctx.fillStyle = SPEC_COLORS.accentOverlayStrong;
        ctx.beginPath();
        ctx.moveTo(dx, dy - 7);
        ctx.lineTo(dx + 7, dy);
        ctx.lineTo(dx, dy + 7);
      ...
      drawText(ctx, step.label, blx + 24, stepY + 30, FONTS.bodyMed, SPEC_COLORS.accentText, cP);
