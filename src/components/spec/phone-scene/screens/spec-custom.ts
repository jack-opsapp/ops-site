/**
 * SPEC Custom Screen — The finished product, tier-specific.
 *
...
import { COLORS, LAYOUT, SPEC_COLORS, TEXT, CONTENT_PADDING, CARD_PADDING, TOP_INSET } from '../constants';
import {
...
function drawSetupTier({ ctx, width, height, progress }: SpecScreenDrawParams) {
  const cw = width - P * 2;
  const lx = P + CP;
  const rx = P + cw - CP;
  ...
  // Badge
  const badgeText = 'Configured by OPS SPEC';
  const badgeW = 260;
  roundedRect(ctx, P, y, badgeW, 34, 3, SPEC_COLORS.accentGlow, SPEC_COLORS.accentBorder, contentP);
  drawText(ctx, badgeText, P + badgeW / 2, y + 17, FONTS.labelSm, SPEC_COLORS.accentText, contentP, 'center');
  y += 60;
  ...
  stages.forEach((s, i) => {
    ...
    roundedRect(ctx, sx, y, stageW, 42, 3,
      isActive ? SPEC_COLORS.accentGlow : isPast ? 'rgba(165, 179, 104, 0.08)' : COLORS.cardFill,
      isActive ? SPEC_COLORS.accentBorder : isPast ? 'rgba(165, 179, 104, 0.25)' : COLORS.border,
      structP,
    );
    drawText(ctx, s, sx + stageW / 2, y + 22, FONTS.labelXs,
      isActive ? SPEC_COLORS.accentText : isPast ? COLORS.stageAccepted : TEXT.tertiary,
      contentP, 'center');
  });
  y += 42 + 32;
...
function drawBuildTier({ ctx, width, height, progress }: SpecScreenDrawParams) {
  const cw = width - P * 2;
  const lx = P + CP;
  const rx = P + cw - CP;
  ...
  drawText(ctx, 'DECK DESIGNER', P + 44, y, FONTS.caption, SPEC_COLORS.accentText, structP);
  y += 46;
  ...
  if (contentP > 0) {
    ctx.save();
    ctx.globalAlpha = contentP;
    ctx.fillStyle = SPEC_COLORS.accentGlow;
    ctx.beginPath();
    ctx.roundRect(deckX, deckY, deckW, deckH, 4);
    ctx.fill();
    ctx.strokeStyle = SPEC_COLORS.accentBorder;
    ctx.lineWidth = 2;
    ctx.stroke();
    ctx.restore();
  }
  ...
  if (detailP > 0) {
    ...
    ctx.strokeStyle = SPEC_COLORS.accentText;
    ctx.lineWidth = 1;
  ...
  if (detailP > 0) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      ctx.fillStyle = SPEC_COLORS.accentOverlay;
      ctx.strokeStyle = SPEC_COLORS.accentBorder;
      ctx.lineWidth = 1;
  ...
  if (accentP > 0) {
    ...
    drawText(ctx, "16'", deckX + deckW / 2, hDimY - 16, FONTS.caption, SPEC_COLORS.accentText, 1, 'center');
    ...
    drawText(ctx, "12'", 0, 0, FONTS.caption, SPEC_COLORS.accentText, 1, 'center');
    ctx.restore();
    ctx.restore();
  }
  ...
  tools.forEach((name, i) => {
    ...
    roundedRect(ctx, tx, y, toolW, toolH, 4,
      isActive ? SPEC_COLORS.accentGlow : COLORS.cardFill,
      isActive ? SPEC_COLORS.accentBorder : COLORS.border,
      contentP,
    );
    if (contentP > 0) {
      ...
      ctx.save();
      ctx.globalAlpha = contentP;
      ctx.strokeStyle = isActive ? SPEC_COLORS.accentText : TEXT.tertiary;
      ctx.lineWidth = 1.5;
      if (i === 0) {
    ...
    drawText(ctx, name, tx + toolW / 2, y + 48, FONTS.labelXs,
      isActive ? SPEC_COLORS.accentText : TEXT.tertiary, contentP, 'center');
  });
  y += toolH + 22;
  ...
  // Estimate Bar
  const estH = 100;
  roundedRect(ctx, P, y, cw, estH, LAYOUT.cardRadius, SPEC_COLORS.accentGlow, SPEC_COLORS.accentBorder, accentP);
  drawText(ctx, 'Est. Materials', lx, y + 34, FONTS.labelSm, TEXT.secondary, accentP);
  drawText(ctx, '$4,847', lx, y + 72, FONTS.price, SPEC_COLORS.accentText, accentP);
  drawText(ctx, 'Generate Quote \u2192', rx, y + 54, FONTS.caption, SPEC_COLORS.accentText, accentP, 'right');
...
function drawEnterpriseTier({ ctx, width, height, progress }: SpecScreenDrawParams) {
  const cw = width - P * 2;
  const lx = P + CP;
  const rx = P + cw - CP;
  ...
  tabs.forEach((t, i) => {
    ...
    if (isActive && contentP > 0) {
      ctx.save();
      ctx.globalAlpha = contentP;
      ctx.fillStyle = SPEC_COLORS.accentText;
      ctx.fillRect(tabX, y + 22, tw - 16, 2);
      ctx.restore();
    }
    drawText(ctx, t, tabX, y + 8, FONTS.labelSm,
      isActive ? SPEC_COLORS.accentText : TEXT.tertiary, contentP);
    tabX += tw;
  });
  y += 52;
...
export function drawSpecCustom(params: SpecScreenDrawParams) {
